...
function _xml_parse( dom ) {
  try {
    return dom.multistatus.response.map( function ( response ) {
      ...
      if ( propstat ) {
        var prop = propstat.prop ? propstat.prop[0] : null

        if ( prop ) {
          var getlastmodified = prop.getlastmodified ? prop.getlastmodified[0] : Date.now()
          var getcontentlength = prop ? prop.getcontentlength : null
          ...
          var resourcetype = prop.resourcetype
          if ( resourcetype ) {
            if ( typeof resourcetype[0] === 'string' ) {
              resource.resourcetype = null
            } else if ( typeof resourcetype[0] === 'object' ) {
              resource.resourcetype = Object.keys( resourcetype[0] )[0]
            }
          }
        }

        var stat = propstat.status ? propstat.status[0] : null
        if ( stat ) {
